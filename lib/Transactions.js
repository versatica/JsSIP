...
class NonInviteClientTransaction extends events.EventEmitter {
  constructor(request_sender, request, transport) {
    super();
  ...
  send() {
    ...
    this.F = setTimeout(() => {tr.timer_F();}, Timers.TIMER_F);
  ...
  receiveResponse(response) {
    const tr = this, status_code = response.status_code;
    ...
    if(status_code < 200) {
      ...
      switch(this.state) {
        ...
        case C.STATUS_PROCEEDING:
          ...
          this.K = setTimeout(() => {tr.timer_K();}, Timers.TIMER_K);
          break;
...
class InviteClientTransaction extends events.EventEmitter {
  constructor(request_sender, request, transport) {
    super();
  ...
  sendACK(response) {
    ...
    this.D = setTimeout(() => {tr.timer_D();}, Timers.TIMER_D);

    this.transport.send(this.ack);
  }

  cancel_request(tr, reason) {
...
class AckClientTransaction extends events.EventEmitter {
  constructor(request_sender, request, transport) {
    super();
...
class NonInviteServerTransaction extends events.EventEmitter {
...
class InviteServerTransaction extends events.EventEmitter {
  constructor(request, ua) {
    super();
 ...
 *  _false_ new request
 */
function checkTransaction({transactions}, request) {
  let tr;

  switch(request.method) {
    case JsSIP_C.INVITE:
      tr = transactions.ist[request.via_branch];
      if(tr) {
        switch(tr.state) {
          case C.STATUS_PROCEEDING:
    ...
    default:
      ...
      tr = transactions.nist[request.via_branch];
      if(tr) {
        switch(tr.state) {
          case C.STATUS_TRYING:
            break;
