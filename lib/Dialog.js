...
class Dialog {
  constructor(owner, message, type, state = C.STATUS_CONFIRMED) {
    ...
    let contact = message.parseHeader('contact');
  ...
  // RFC 3261 12.2.1.1
  createRequest(method, extraHeaders, body) {
    extraHeaders = extraHeaders && extraHeaders.slice() || [];

    if(!this.local_seqnum) { this.local_seqnum = Math.floor(Math.random() * 10000); }

    let cseq = (method === JsSIP_C.CANCEL || method === JsSIP_C.ACK) ? this.local_seqnum : this.local_seqnum += 1;

    let request = new SIPMessage.OutgoingRequest(
      method,
      this.remote_target,
      this.owner.ua, {
        'cseq': cseq,
  ...
  checkInDialogRequest(request) {
    ...
    if (request.method === JsSIP_C.INVITE || (request.method === JsSIP_C.UPDATE && request.body)) {
      if (this.uac_pending_reply === true) {
        ...
        request.server_transaction.once('stateChanged', () => {
  ...
  sendRequest(applicant, method, options = {}) {
    ...
    const request_sender = new Dialog_RequestSender(this, applicant, request);
