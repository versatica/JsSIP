...
// RFC 3261 12.1
class Dialog {
  constructor(owner, message, type, state) {
    let contact;
  ...
  sendRequest(applicant, method, options) {
    ...
    const extraHeaders = options.extraHeaders && options.extraHeaders.slice() || [], body = options.body || null, request = this.createRequest(method, extraHeaders, body), request_sender = new Dialog_RequestSender(this, applicant, request);
