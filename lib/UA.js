...
module.exports = class UA extends EventEmitter
{
  ...
  set(parameter, value)
  {
    switch (parameter)
    {
      ...
      case 'contentTypeShareStateRegexp':
      case 'contentTypeEnterRoomRegexp': {
  ...
  receiveRequest(request)
  {
    ...
    else if (method === JsSIP_C.INFO)
    {
      ...
      if (contentType &&
        this._configuration.contentTypeShareStateRegexp &&
        contentType.match(this._configuration.contentTypeShareStateRegexp)
      )
      {
        ...
        contentType &&
        this._configuration.contentTypeEnterRoomRegexp &&
        contentType.match(this._configuration.contentTypeEnterRoomRegexp)
      )
      {
  ...
  _loadConfig(configuration)
  {
    ...
    const writable_parameters = [
      'password', 'realm', 'ha1', 'display_name', 'register', 'contentTypeShareStateRegexp', 'contentTypeEnterRoomRegexp'
    ];
