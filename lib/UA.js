...
module.exports = class UA extends EventEmitter
{
  ...
  {
    const debugPrefix = configuration.debug_prefix? `${configuration.debug_prefix}:` : '';

    super();

    this.debug = require('debug')(`${debugPrefix}JsSIP:UA`);
    this.debugerror = require('debug')(`${debugPrefix}JsSIP:ERROR:UA`);
    this.debugerror.log = console.warn.bind(console);

    this.debug('new() [configuration:%o]', configuration);
  ...
  receiveRequest(request)
  {
    ...
    if (this._configuration.check_user && 
      ...
      if (Object.prototype.hasOwnProperty.call(config.settings, parameter))
      {
        switch (parameter)
        {
          ...
          case 'registrar_server':
            this.debug(`- ${parameter}: ${this._configuration[parameter]}`);
            break;
          case 'password':
          case 'ha1':
            this.debug(`- ${parameter}: NOT SHOWN`);
            break;
          default:
            this.debug(`- ${parameter}: ${JSON.stringify(this._configuration[parameter])}`);
        }
      }
    }
