...
class UA extends events.EventEmitter {
   ...
   */
  findSession(request) {
    const sessionIDa = request.call_id + request.from_tag;
    const sessionA = this.sessions[sessionIDa];
    const sessionIDb = request.call_id + request.to_tag;
...
function onTransportDisconnect(data) {
  ...
  for (let type of client_transactions) {
    for(let transaction in this.transactions[type]) {
