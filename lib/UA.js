...
const WebRTCGlobals = require('./WebRTCGlobals');
...
module.exports = class UA extends EventEmitter
{
  ...
  constructor(configuration)
  {
    ...
    if (configuration.webRTCOverrides)
    {
      // eslint-disable-next-line max-len
      WebRTCGlobals.getInstance().RTCPeerConnection = configuration.webRTCOverrides.RTCPeerConnection;
      // eslint-disable-next-line max-len
      WebRTCGlobals.getInstance().getUserMedia = configuration.webRTCOverrides.getUserMedia;
      // eslint-disable-next-line max-len
      WebRTCGlobals.getInstance().getDisplayMedia = configuration.webRTCOverrides.getDisplayMedia;
      // eslint-disable-next-line max-len
      WebRTCGlobals.getInstance().enumerateDevices = configuration.webRTCOverrides.enumerateDevices;
    }
  ...
  receiveRequest(request)
  {
    ...
    if (!request.to_tag)
    {
      ...
      {
        case JsSIP_C.INVITE:
          if (WebRTCGlobals.getInstance().RTCPeerConnection)
          { // TODO
