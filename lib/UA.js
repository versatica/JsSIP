 ...
 */
class UA extends events.EventEmitter {
  ...
  call(target, options) {
    ...
    let session = new RTCSession(this);
  ...
  terminateSessions(options) {
    ...
    for(const idx in this.sessions) {
  ...
  stop() {
    ...
    const num_sessions = Object.keys(this.sessions).length;
  ...
  {
    return this.nictTransactionsCount() +
      this.nistTransactionsCount() +
      this.ictTransactionsCount() +
      this.istTransactionsCount();
  }

  get nictTransactionsCount()
  {
    return Object.keys(this.transactions.nict).length;
  }

  get nistTransactionsCount()
  {
    return Object.keys(this.transactions.nist).length;
  }

  get ictTransactionsCount()
  {
    return Object.keys(this.transactions.ict).length;
  }

  get istTransactionsCount()
  {
   ...
   */
  findSession({call_id, from_tag, to_tag}) {
    const sessionIDa = call_id + from_tag;
    const sessionA = this.sessions[sessionIDa];
    const sessionIDb = call_id + to_tag;
    const sessionB = this.sessions[sessionIDb];
   ...
   */
  findDialog(call_id, from_tag, to_tag) {
  ...
  loadConfig(configuration) {
    ...
    for(parameter in UA.configuration_check.mandatory) {
      if(!configuration.hasOwnProperty(parameter)) {
        ...
        let value = configuration[parameter];
        let checked_value = UA.configuration_check.mandatory[parameter].call(this, value);
    ...
    for(parameter in UA.configuration_check.optional) {
      if(configuration.hasOwnProperty(parameter)) {
        ...
        let checked_value = UA.configuration_check.optional[parameter].call(this, value, configuration);
        if (checked_value !== undefined) {
          settings[parameter] = checked_value;
        } else {
          throw new Exceptions.ConfigurationError(parameter, value);
    ...
    let hostport_params = settings.uri.clone();
    ...
    if (!settings.registrar_server) {
      let registrar_server = settings.uri.clone();
  ...
  optional: {
    ...
    connection_recovery_max_interval(connection_recovery_max_interval) {
      if(Utils.isDecimal(connection_recovery_max_interval)) {
        let value = Number(connection_recovery_max_interval);
        if(value > 0) {
          return value;
        }
      }
    ...
    connection_recovery_min_interval(connection_recovery_min_interval) {
      if(Utils.isDecimal(connection_recovery_min_interval)) {
        let value = Number(connection_recovery_min_interval);
        if(value > 0) {
          return value;
        }
      }
    ...
    no_answer_timeout(no_answer_timeout) {
      if (Utils.isDecimal(no_answer_timeout)) {
        let value = Number(no_answer_timeout);
        if (value > 0) {
          return value;
        }
      }
    ...
    register_expires(register_expires) {
      if (Utils.isDecimal(register_expires)) {
        let value = Number(register_expires);
        if (value > 0) {
          return value;
        }
      }
    ...
    registrar_server(registrar_server) {
      ...
      let parsed = URI.parse(registrar_server);
...
function onTransportDisconnect(data) {
  ...
  for (let type of client_transactions) {
    for(let id in this.transactions[type]) {
      this.transactions[type][id].onTransportError();
    }
  }
