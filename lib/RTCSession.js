...
const RTCSession = module.exports = class RTCSession extends events.EventEmitter
{
  constructor(ua)
  {
    debug('new');
  ...
  connect(target, options, initCallback)
  {
    ...
    this.request = new SIPMessage.OutgoingRequest(
      JsSIP_C.INVITE, target, this.ua, requestParams, extraHeaders);
  ...
  close()
  {
    ...
    {
      if (Object.prototype.hasOwnProperty.call(this.timers, timer))
      {
    ...
    for (const dialog in this.earlyDialogs)
    {
      if (Object.prototype.hasOwnProperty.call(this.earlyDialogs, dialog))
      {
  ...
  sendDTMF(tones, options = {})
  {
    ...
    function _sendDTMF()
    {
      ...
      if (this.status === C.STATUS_TERMINATED ||
          !this.tones || position >= this.tones.length)
      {
        // Stop sending DTMF
        this.tones = null;
      ...
      const tone = this.tones[position];
...
function receiveRefer(request)
{
  ...
  function accept(initCallback, options = {})
  {
    ...
    const session = new RTCSession(this.ua);
