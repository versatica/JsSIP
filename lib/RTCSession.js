...
module.exports = class RTCSession extends EventEmitter
{
  ...
  replaceMediaStream(stream, { deleteExisting = true, addMissing = true } = {})
  {
  ...
  _addMediaStream(stream, action = 'getTracks')
  {
    stream[action]().forEach((track) => this._connection.addTrack(track, stream));
  }
  ...
  _addPresentationStream(stream)
  {
    this._presentationStream = stream;
  }

  _removePresentationStream()
  {
    this._presentationStream = null;
  }

  _isPresentationStreamTrack(track)
  {
    return this._presentationStream &&
      this._presentationStream.getTracks().some((presentationTrack) =>
      {
        return track === presentationTrack;
      });
  }

  presentation(isStart, stream, { isNeedReinvite=true }={})
  {
    debug('presentation()');
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      {
        this.emit('presentation:failed', error);
      ...
      const header = isStart ? 'LETMESTARTPRESENTATION' : 'STOPPRESENTATION';
      ...
      this.sendRequest(JsSIP_C.INFO, {
        extraHeaders  : [ `X-WEBRTC-SHARE-STATE: ${header}` ],
        body          : '',
        eventHandlers : {
          onSuccessResponse : (response) =>
          {
            if (response.reason_phrase === 'OK')
            {
              if (isStart)
              {
                this._addPresentationStream(stream);
                this._addMediaStream(stream, 'getVideoTracks');
              }
              else
              {
                this._removePresentationStream();
  ...
  _toggleMuteAudio(mute)
  {
    ...
    {
      if (track && track.kind === 'audio' && !this._isPresentationStreamTrack(track))
      {
        track.enabled = !mute;
      }
    });
  ...
  _toggleMuteVideo(mute)
  {
    this._forEachSenders(({ track }) =>
    {
      if (track && track.kind === 'video' && !this._isPresentationStreamTrack(track))
      {
        track.enabled = !mute;
      }
    });
