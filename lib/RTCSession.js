...
module.exports = class RTCSession extends EventEmitter
{
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    if (this._ua.configuration.auth && extraHeaders.filter((extraHeader) => extraHeader.startsWith('Authorization: Bearer')).length==0)
    {
      extraHeaders.push(`Authorization: Bearer ${this._ua.configuration.auth}`);
    }   
    // Check target.
    if (target === undefined)
    {
