...
module.exports = class RTCSession extends EventEmitter
{
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    this._sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream, options.onAddedSender);
  }
  ...
  answer(options = {})
  {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          return this._addMediaStream(stream, 'getTracks', options.onAddedSender);
        }
      })
  ...
  _addMediaStream(stream, action, onAddedSender)
  {
    return Promise.all(stream[action]().map((track) =>
    {
      ...
      if (onAddedSender)
      {
        return onAddedSender(sender, track, stream);
      }
  ...
  _addOrReplacePresentationMediaStream(stream, onAddedSender)
  {
    ...
    {
      return this._addMediaStream(stream, 'getVideoTracks', onAddedSender);
    }
  }
  ...
  startPresentation(stream, isNeedReinvite=true, onAddedSender)
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      Promise.resolve().then(() =>
      {
        ...
        {
          return this._addOrReplacePresentationMediaStream(stream, onAddedSender);
        }
   ...
   */
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream, onAddedSender)
  {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          return this._addMediaStream(stream, 'getTracks', onAddedSender);
        }
