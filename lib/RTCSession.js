...
module.exports = class RTCSession extends EventEmitter
{
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
        ...
        if ((connection.iceGatheringState === 'complete' && !iceRestart) ||
          (connection.iceGatheringState === 'gathering' && this._iceReady && !iceRestart))
        {
          this._rtcReady = true;
  ...
  _requireIceRestart(sdp)
  {
    const cur_sdp = sdp_transform.parse(this._connection.currentLocalDescription.sdp);
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    const rtcAnswerConstraints = this._rtcAnswerConstraints || {};
