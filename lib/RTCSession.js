...
module.exports = class RTCSession extends EventEmitter
{
  ...
  _isMediaStreamCompatibleWithConnection(mediaStream)
  {
    const kindsTracksConnection = this._connection.getSenders().map((sender) => 
    {
      return sender.track.kind;
    })
      .sort();
    const kindsTracksMediaStream = mediaStream.getTracks().map((track) => 
    {
      return track.kind;
    })
      .sort();

    return kindsTracksConnection.length === kindsTracksMediaStream.length && 
           kindsTracksConnection.every((value, index) => 
           {
             return value === kindsTracksMediaStream[index]; 
           });
  }
  ...
  replaceMediaStream(mediaStream)
  {
    ...
    if (!this._isMediaStreamCompatibleWithConnection(mediaStream))
    {
      throw new TypeError('mediaStream is not compatible with the current stream in the connection');
    }
