...
module.exports = class RTCSession extends EventEmitter
{
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    else
    {
      if (options.fromUri)
      {
        if (options.fromUri instanceof URI)
        {
          requestParams.from_uri = options.fromUri;
        }
        else
        {
          requestParams.from_uri = URI.parse(options.fromUri);
        }
      }
    ...
    // From-header has been modified
    if (anonymous || options.fromUri)
    {
