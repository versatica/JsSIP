...
class RTCSession extends events.EventEmitter {
  ...
  answer(options = {}) {
    ...
    function userMediaSucceeded(stream) {
      if (this.status === C.STATUS_TERMINATED) { return; }
    ...
    function remoteDescriptionSucceededOrNotNeeded() {
      ...
      if (! this.late_sdp) {
        ...
        createLocalDescription.call(this, 'offer', rtcSucceeded.bind(this), rtcFailed.bind(this), this.rtcOfferConstraints);
      }
    }
  ...
  close() {
    ...
    for(let dialog in this.earlyDialogs) {
      this.earlyDialogs[dialog].terminate();
      delete this.earlyDialogs[dialog];
    }
  ...
  sendDTMF(tones, options = {}) {
    ...
    function _sendDTMF() {
      ...
      if (this.status === C.STATUS_TERMINATED || !this.tones || position >= this.tones.length) {
        // Stop sending DTMF
        this.tones = null;
        return;
      }

      tone = this.tones[position];
      position += 1;
  ...
  refer(target, options) {
    ...
    referSubscriber.on('requestFailed', () => {
      delete this.referSubscribers[id];
    });
...
function setInvite2xxTimer(request, body) {
  ...
  function invite2xxRetransmission() {
    ...
    this.timers.invite2xxTimer = setTimeout(
...
function createDialog(message, type, early) {
  ...
  let early_dialog = this.earlyDialogs[id];
  ...
  // Confirmed Dialog
    ...
    let dialog = new Dialog(this, message, type);
...
function receiveReinvite(request) {
  ...
  if (request.body) {
    ...
    for (let m of sdp.media) {
      ...
      let direction = m.direction || sdp.direction || 'sendrecv';
  ...
  function createSdp(onSuccess, onFailure) {
    if (! this.late_sdp) {
      if (this.remoteHold === true && hold === false) {
        this.remoteHold = false;
        onunhold.call(this, 'remote');
      } else if (this.remoteHold === false && hold === true) {
        this.remoteHold = true;
        onhold.call(this, 'remote');
      }

      createLocalDescription.call(this, 'answer', onSuccess, onFailure, this.rtcAnswerConstraints);
    } else {
      createLocalDescription.call(this, 'offer', onSuccess, onFailure, this.rtcOfferConstraints);
    }
  }
}
...
function receiveRefer(request) {
  ...
  function accept(initCallback, options = {}) {
    ...
    let session = new RTCSession(this.ua);
    ...
    if (request.refer_to.uri.hasHeader('replaces')) {
      let replaces = decodeURIComponent(request.refer_to.uri.getHeader('replaces'));
...
function sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream) {
  ...
  function rtcSucceeded(desc) {
    if (this.isCanceled || this.status === C.STATUS_TERMINATED) { return; }
...
function receiveInviteResponse(response) {
  ...
  if (this.dialog && (response.status_code >=200 && response.status_code <=299)) {
    ...
    // If not, send an ACK  and terminate
      ...
      let dialog = new Dialog(this, response, 'UAC');
  ...
  switch(true) {
    ...
    case /^1[0-9]{2}$/.test(response.status_code):
      ...
      let e = {originator:'remote', type:'answer', sdp:response.body};

      debug('emit "sdp"');

      this.emit('sdp', e);

      let answer = new RTCSessionDescription({type:'answer', sdp:e.sdp});
    ...
    default:
      let cause = Utils.sipErrorCause(response.status_code);
