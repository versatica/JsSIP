...
const IS_SUPPORT_ADD_TRANSCEIVER = !isFirefoxOrLower(78);


const addTrackTransceiver = (connection, track, stream, {
  directionAudio,
  directionVideo,
  sendEncodings,
  onAddedTransceiver
}) =>
{
  ...
  {
    transceiver = connection.addTransceiver(track, { direction, sendEncodings, streams: [ stream ] });
  }
  else
  {
    connection.addTrack(track, stream);
    transceiver = connection.getTransceivers().find((t) => t.sender.track === track);
  }
...
const addTransceiver = (connection, stream, action, {
  directionAudio,
  directionVideo,
  sendEncodings,
  onAddedTransceiver
}) =>
{
  ...
  {
    return addTrackTransceiver(connection, track, stream, {
      directionAudio,
      directionVideo,
      sendEncodings,
      onAddedTransceiver
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  } = {})
  {
    ...
    const sequentReplaceTracks = stream.getTracks().map((track) => () =>
    {
      ...
      if (!sender && addMissing)
      {
        ...
        return addTrackTransceiver(this._connection, track, stream, {
          directionAudio,
          directionVideo,
          sendEncodings,
          onAddedTransceiver
  ...
  {
    return addTransceiver(this._connection, stream, action, {
      directionAudio,
      directionVideo,
      sendEncodings,
      onAddedTransceiver
  ...
  _addOrReplacePresentationMediaStream(stream, { directionAudio, directionVideo, sendEncodings, onAddedTransceiver })
  {
