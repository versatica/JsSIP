  ...
  answer(options = {}) {
    ...
    // If at least audio or video is requested prompt getUserMedia.
      ...
      navigator.mediaDevices.getUserMedia(mediaConstraints)
        .then(userMediaSucceeded.call(this))
        .catch(error => {
    ...
    function userMediaSucceeded(stream) {
      ...
      if (! this.late_sdp) {
        ...
        this.connection.setRemoteDescription(offer)
          .then(remoteDescriptionSucceededOrNotNeeded.call(this))
          .catch(error => {
            request.reply(488);
    ...
    function remoteDescriptionSucceededOrNotNeeded() {
      ...
      if (! this.late_sdp) {
        createLocalDescription.call(this, 'answer', rtcSucceeded, rtcFailed, rtcAnswerConstraints);
      } else {
        createLocalDescription.call(this, 'offer', rtcSucceeded, rtcFailed, this.rtcOfferConstraints);
      }
    }
...
function createLocalDescription(type, onSuccess, onFailure, constraints) {
  ...
  if (type === 'offer') {
    connection.createOffer(constraints)
      .then(createSucceeded.call(this))
      .catch(error => {
        this.rtcReady = true;
...
function sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream) {
  ...
  function userMediaSucceeded(stream) {
    ...
    createLocalDescription.call(this, 'offer', rtcSucceeded, rtcFailed, rtcOfferConstraints);
  }
