...
const setEncodingsToSender=(options) =>
{

  if (options.sendEncodings)
  {
    const parametersCurrent = options.sender.getParameters();

    parametersCurrent.encodings = options.sendEncodings;

    return options.sender.setParameters(parametersCurrent);
  }
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  answer(options = {})
  {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          return this._addMediaStreamInTransceiver(stream, 'getTracks', { sendEncodings: options.sendEncodings, onAddedTransceiver: options.onAddedTransceiver });
        }
      })
  ...
  } = {})
  {
    ...
    const sequentReplaceTracks = stream.getTracks().map((track) => () =>
    {
      ...
      if (!sender && addMissing)
      {
        ...
        const transceiver = this._connection.addTransceiver(track, { sendEncodings, streams: [ stream ] });
  ...
  _addMediaStreamInTransceiver(stream, action, { direction, sendEncodings, onAddedTransceiver })
  {
    return Promise.all(stream[action]().map((track) =>
    {
      const transceiver = this._connection.addTransceiver(track, { direction, sendEncodings, streams: [ stream ] });
  ...
  _addMediaStreamInSender(stream, action, { sendEncodings, onAddedTransceiver })
  {
    return Promise.all(stream[action]().map((track) =>
    {
      const sender = this._connection.addTrack(track, stream);
      const transceiver = this._connection.getTransceivers().find((transceiver) => transceiver.sender === sender);

      return setEncodingsToSender({ sender, sendEncodings }).then(() =>
      {
  ...
  _addOrReplacePresentationMediaStream(stream, { sendEncodings, onAddedTransceiver })
  {
    ...
    else
    {
      ...
      const isExistRecvOnlyTransceiver = transceivers.some((transceiver) =>
      {
        return transceiver.currentDirection === 'recvonly';
      });

      if (isExistRecvOnlyTransceiver)
      {
        return this._addMediaStreamInSender(stream, 'getVideoTracks', { sendEncodings, onAddedTransceiver });
      }

      return this._addMediaStreamInTransceiver(stream, 'getVideoTracks', { sendEncodings, onAddedTransceiver, direction: 'sendonly' });
    }
  }
  ...
  startPresentation(stream, isNeedReinvite=true, { sendEncodings=undefined, onAddedTransceiver=undefined }={})
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      Promise.resolve().then(() =>
      {
        ...
        {
          return this._addOrReplacePresentationMediaStream(stream, { sendEncodings, onAddedTransceiver });
        }

        return this._addMediaStreamInTransceiver(stream, 'getVideoTracks', { sendEncodings, onAddedTransceiver });
      })
        .then(() =>
        {
   ...
   */
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream, { sendEncodings, onAddedTransceiver })
  {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          return this._addMediaStreamInTransceiver(stream, 'getTracks', { sendEncodings, onAddedTransceiver });
        }
