...
const PureSipConnection = require('./RTCSession/PureSipConnection');
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    this.debug = require('debug')(`${debugPrefix}JsSIP:RTCSession`);
    this.debugerror = require('debug')(`${debugPrefix}JsSIP:ERROR:RTCSession`);
    ...
    this._connectionConfig = {
      localDescriptionFactory : () =>
      {
        throw new Exceptions.NotSupportedError('localDescriptionFactory not set, please call setLocalDescriptionFactory');
      },
      localDescriptionModifier : (description) =>
      {
        return Promise.resolve(description);
      },
      remoteDescriptionModifier : (description) =>
      {
        return Promise.resolve(description);
      }
    };
    this._connectionConfig.enable_webrtc = this._ua.configuration.use_webrtc;
  ...
  setLocalDescriptionFactory(localDescriptionFactory) 
  {
    if (localDescriptionFactory && typeof localDescriptionFactory === 'function') 
    {
      this._connectionConfig.localDescriptionFactory = (type, constraints) => 
      {
  ...
  setLocalDescriptionModifier(localDescriptionModifier) 
  {
    if (localDescriptionModifier && typeof localDescriptionModifier === 'function')
    {
      this._connectionConfig.localDescriptionModifier = (description) =>
      {
  ...
  setRemoteDescriptionModifier(remoteDescriptionModifier) 
  {
    if (remoteDescriptionModifier && typeof remoteDescriptionModifier === 'function')
    {
      this._connectionConfig.remoteDescriptionModifier = (description) =>
      {
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    const call_id = options.call_id || null;
    const display_name = options.display_name || null;
    const from_uri = options.from_uri || null;
  ...
  answer(options = {})
  {
    ...
    else if (mediaConstraints.audio || mediaConstraints.video)
    {
      ...
      Promise.resolve(this._connectionConfig.enable_webrtc ?
        ...
        .catch((error) =>
        {
          ...
          this.debugerror('emit "getusermediafailed" [error:%o]', error);
    ...
    function userMediaSucceeded(stream)
    {
      ...
      if (! this._late_sdp)
      {
        ...
        const offer = this._newRTCSessionDescription({ type: 'offer', sdp: e.sdp });

        this._connectionConfig.remoteDescriptionModifier(offer)
          .then((modOffer) => this._connection.setRemoteDescription(modOffer))
          ...
          .catch((error) =>
          {
            ...
            this.debugerror('emit "peerconnection:setremotedescriptionfailed" [error:%o]', error);
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            const answer = this._newRTCSessionDescription({ type: 'answer', sdp: e.sdp });

            this.emit('sdp', e);

            this._connectionConfig.remoteDescriptionModifier(answer)
              .then((modAnswer) => this._connection.setRemoteDescription(modAnswer))
              .then(() =>
              {
  ...
  _newRTCSessionDescription(sessionDescriptionInit) 
  {
    return this._connectionConfig.enable ? 
      new RTCSessionDescription(sessionDescriptionInit) : 
  ...
  _setACKTimer()
  {
    this._timers.ackTimer = setTimeout(() =>
    {
      if (this._status === C.STATUS_WAITING_FOR_ACK)
      {
        this.debug('no ACK received, terminating the session');
  ...
  _createRTCConnection(pcConfig, rtcConstraints)
  {
    ...
    {
      this._connection = new PureSipConnection((type, constraints) =>
      {
        return this._connectionConfig.localDescriptionFactory(type, constraints);
      });
    }
  }
  ...
  _createLocalDescription(type, onSuccess, onFailure, constraints)
  {
    ...
    if (type === 'offer')
    {
      connection.createOffer(constraints)
        ...
        .catch((error) =>
        {
          ...
          this.debugerror('emit "peerconnection:createofferfailed" [error:%o]', error);
    ...
    else if (type === 'answer')
    {
      connection.createAnswer(constraints)
        ...
        .catch((error) =>
        {
          ...
          this.debugerror('emit "peerconnection:createanswerfailed" [error:%o]', error);
    ...
    function createSucceeded(desc)
    {
      ...
      this._connectionConfig.localDescriptionModifier(desc)
        .then((modDesc) => connection.setLocalDescription(modDesc))
        .then(() =>
        {
        ...
        .catch((error) =>
        {
          ...
          this.debugerror('emit "peerconnection:setlocaldescriptionfailed" [error:%o]', error);
  ...
  _runSessionTimer()
  {
    ...
    if (this._sessionTimers.refresher)
    {
      this._sessionTimers.timer = setTimeout(() =>
      {
        ...
        this.debug('runSessionTimer() | sending session refresh request');
  
        if (this._sessionTimers.useUpdate) 
        {
          this._sendUpdate({
            eventHandlers
          });
    ...
    else
    {
      this._sessionTimers.timer = setTimeout(() =>
      {
        ...
        this.debugerror('runSessionTimer() | timer expired, terminating the session');
