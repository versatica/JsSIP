...
class OutgoingRequest {
  constructor(method, ruri, ua, params, extraHeaders, body) {
    ...
    // Call-ID
    let call_id = params.call_id || (ua.configuration.jssip_id + Utils.createRandomToken(15));
    this.call_id = call_id;
    this.setHeader('call-id', call_id);

    // CSeq
    let cseq = params.cseq || Math.floor(Math.random() * 10000);
    this.cseq = cseq;
  ...
  toString() {
    ...
    if (this.body) {
      let length = Utils.str_utf8_length(this.body);
...
class IncomingMessage {
  ...
  */
  parseHeader(name, idx = 0) {
    ...
    let header = this.headers[name][idx];
    ...
    let parsed = Grammar.parse(value, name.replace(/-/g, '_'));
