...
module.exports = class Request
{
  constructor(session, method)
  {
    ...
    if (this.session.status !== RTCSession_C.STATUS_1XX_RECEIVED &&
      this.session.status !== RTCSession_C.STATUS_WAITING_FOR_ANSWER &&
      this.session.status !== RTCSession_C.STATUS_WAITING_FOR_ACK &&
      this.session.status !== RTCSession_C.STATUS_CONFIRMED &&
      this.session.status !== RTCSession_C.STATUS_TERMINATED)
    {
     ...
     */
    else if (this.session.status === RTCSession_C.STATUS_TERMINATED &&
        method !== JsSIP_C.BYE)
    {
      throw new Exceptions.InvalidStateError(this.session.status);
    }
  ...
  send(options = {})
  {
    ...
    this.outgoingRequest = this.session._dialog.sendRequest(this, this.method, {
  ...
  receiveResponse(response)
  {
    switch (true)
    {
      case /^1[0-9]{2}$/.test(response.status_code):
        ...
        if (this.eventHandlers.onProgressResponse)
        {
          this.eventHandlers.onProgressResponse(response);
        }
        break;

      case /^2[0-9]{2}$/.test(response.status_code):
        debug('onSuccessResponse');
        if (this.eventHandlers.onSuccessResponse)
        {
          this.eventHandlers.onSuccessResponse(response);
        }
        break;

      default:
        debug('onErrorResponse');
        if (this.eventHandlers.onErrorResponse)
        {
          this.eventHandlers.onErrorResponse(response);
        }
        break;
    }
  }
