  ...
  */
  close: function() {
    ...
    if (this.state !== 'terminated') {
      JsSIP.log(JsSIP.C.LOG_SUBSCRIBER,'terminating Subscriber');
  ...
  */
  ...
  subscribe: function() {
    ...
    if (['notify_wait', 'pending', 'active', 'terminated'].indexOf(this.state) !== -1) {
      JsSIP.error(JsSIP.C.LOG_SUBSCRIBER,'subscription is already on');
      return;
    }
        ...
        switch(true) {
          ...
          case /^2[0-9]{2}$/.test(response.status_code):
            ...
            if (expires && expires <= subscriber.expires) {
              ...
              if (!expires) {
                JsSIP.warn(JsSIP.C.LOG_SUBSCRIBER,'Expires header missing in a 200-class response to SUBSCRIBE');
                subscriber.onFailure(null, JsSIP.C.EXPIRES_HEADER_MISSING);
              } else {
                JsSIP.warn(JsSIP.C.LOG_SUBSCRIBER,'Expires header in a 200-class response to SUBSCRIBE with a higher value than the indicated in the request');
    ...
    switch (subscription_state.state) {
      ...
      case 'terminated':
        if (subscription_state.reason) {
          JsSIP.log(JsSIP.C.LOG_SUBSCRIBER,'terminating subscription with reason '+ subscription_state.reason);
        }
        window.clearTimeout(this.N);
        this.close();
        break;
  ...
  */
  matchEvent: function(request) {
    ...
    if (!request.hasHeader('Event')) {
      JsSIP.warn(JsSIP.C.LOG_SUBSCRIBER,'missing Event header');
      return false;
    }
    // Check mandatory header Subscription-State
    if (!request.hasHeader('Subscription-State')) {
      JsSIP.warn(JsSIP.C.LOG_SUBSCRIBER,'missing Subscription-State header');
      return false;
    }
    ...
    if (this.event !== event) {
      JsSIP.warn(JsSIP.C.LOG_SUBSCRIBER,'event match failed');
  ...
  */
  receiveRequest: function(request, initial) {
    ...
    if (!initial && !this.subscriber.matchEvent(request)) {
      JsSIP.warn(JsSIP.C.LOG_SUBSCRIBER,'NOTIFY request does not match event');
      return;
    }
